...
CREATE TABLE GROUPS(
    ...
    MID VARCHAR2(100) REFERENCES MEMBER(MID)); -- 그룹 만든 멤버의 ID
...
DROP SEQUENCE EVENTDETAIL_SEQ; --EVENT 에 참여한 MEMBER 의 SHARE
...
CREATE TABLE CENTRALSYSTEM (
    ...
    CSAMOUNT NUMBER(15) NOT NULL);
    
COMMIT;

INSERT INTO CENTRALSYSTEM
 VALUES(CENTRALSYSTEM_SEQ.NEXTVAL, (SELECT SUM(-GMBALANCE) FROM GROUPDETAIL WHERE GMBALANCE < 0 AND GID=1));
...
-- 돈내라는 이메일을 보낼 멤버 아이디들
SELECT MID FROM GROUPDETAIL WHERE GMBALANCE <0;


-- [8] SPLIT
DROP SEQUENCE SPLITSUMMARY_SEQ;

CREATE SEQUENCE SPLITSUMMARY_SEQ MAXVALUE 99999999 NOCYCLE NOCACHE;

DROP TABLE SPLITSUMMARY;

CREATE TABLE SPLITSUMMARY(
    SSID VARCHAR2(100)  PRIMARY KEY,
    MID VARCHAR2(100)  REFERENCES MEMBER(MID) NOT NULL,
    GID  REFERENCES GROUPS(GID) NOT NULL,
    PAYAMOUNT NUMBER(15) DEFAULT 0,
    GETAMOUNT NUMBER(15) DEFAULT 0,
