...
public class Member_dao {
	...
	public void adMember(String mid, int ad_email, int ad_phone, int ad_call) {
		String sql = "INSERT INTO AD (MID,AD_EMAIL,AD_PHONE,AD_CALL) " + "VALUES (?,?,?,?)";
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = getConnection();
	...
	public Member_dto getMember(String mid) {
		...
		try {
			...
			if (rs.next()) {
				...
				member = new Member_dto(mid, mpw, mname, mphone, maddress_basic, maddress_detail, mbirth, memail,
						mgender, mpoint, mcumulative_buy, mjoindate, mstatus, gname, cart_cnt, ad_email, ad_phone,
						ad_call);
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
	...
	public ArrayList<Member_dto> getAllMember() {
		...
		try {
			...
			while (rs.next()) {
				...
				int ad_phone = rs.getInt("ad_phone");
				int ad_call = rs.getInt("ad_call");
				members.add(new Member_dto(mid, mpw, mname, mphone, maddress_basic, maddress_detail, mbirth, memail,
						mgender, mpoint, mcumulative_buy, mjoindate, mstatus, gname, cart_cnt, ad_email, ad_phone,
						ad_call));
			}
		} catch (SQLException e) {
	...
	public boolean memberModify(String mid, String mpw, String mname, String mphone, String maddress_basic,
			String maddress_detail, Date mbirth, String memail, String mgender) {
		boolean result = false;
