...
import com.lec.dto.Payment_dto;
...
public class Payment_dao {
	...
	public Payment_dto getPaymentOno(int ono) {
		...
		try {
			...
			if (rs.next()) {
				...
				payment = new Payment_dto(pano, ono, product_all_pay, discount, coupondiscount, usepoint,
						purchase_amount);
			}
	...
	public boolean addPayment(int ono, int product_all_pay, int discount, int coupondiscount, int usepoint,
			int purchase_amount) {
