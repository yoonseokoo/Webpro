...
public class Customer_service_dao {
	...
	public ArrayList<Customer_service_dto> getCustomer_serviceList(int startRow, int endRow) {
		...
		try {
			...
			while (rs.next()) {
				...
				csboards.add(new Customer_service_dto(cno, mid, mname, csubject, ccontent, csecret, cimage, chit,
						cgroup, cgroup_outnum, cindent, crdate, cmexist, ono));
			}
	...
	public Customer_service_dto getCsBoard(int cno) {
		...
		try {
			...
			if (rs.next()) {
				...
				boolean cmexist = rs.getInt("cmexist") == 1;
				String ono = rs.getString("ono");
				csboard = new Customer_service_dto(cno, mid, mname, csubject, ccontent, csecret, cimage, chit, cgroup,
						cgroup_outnum, cindent, crdate, cmexist, ono);
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
	...
	public boolean writeCsBoardMember(String mid, String csubject, String ccontent, boolean csecret, String cimage,
			int ono) {
	...
	public boolean modifyCsBoardMember(int cno, String csubject, String ccontent, boolean csecret, String cimage,
			int ono) {
		boolean result = false;
	...
	public boolean csReplyStepB(String mid, String csubject, String ccontent, boolean csecret, String cimage,
			int cgroup, int cgroup_outnum, int cindent, int ono) {
