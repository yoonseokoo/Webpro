DROP TABLE PAYMENT;
DROP TABLE COUPON;
DROP TABLE CSMG_COMMENT;
DROP TABLE REVIEW_COMMENT;
DROP TABLE REVIEW_RGOOD;
DROP TABLE CART;
DROP TABLE REVIEW;
...
DROP SEQUENCE PANO_SEQ;
DROP SEQUENCE CONO_SEQ;
...
CREATE SEQUENCE RCNO_SEQ
    ...
    NOCYCLE
    NOCACHE;
CREATE SEQUENCE CSNO_SEQ --CS_MGCOMMENT
    MAXVALUE 999999
    NOCYCLE
    NOCACHE;
CREATE SEQUENCE CONO_SEQ --COUPON
    MAXVALUE 999999
    NOCYCLE
    NOCACHE;
...
CREATE TABLE MEMBER(
    ...
    GNO NUMBER(1) REFERENCES GRADES(GNO));

CREATE TABLE AD(
    MID VARCHAR2(30) PRIMARY KEY,
    AD_EMAIL NUMBER(1) DEFAULT 0,  -- 1?ù¥ ?àò?ùΩ 0?ù¥ Í±∞Ï†à
...
CREATE TABLE CUSTOMER_SERVICE(
    ...
    CSECRET NUMBER(1) DEFAULT 0,  --1?ù¥ Í≥µÍ∞ú
    CIMAGE VARCHAR2(30),
    ...
    CMEXIST NUMBER(1) DEFAULT 0,
    ONO NUMBER(6)
);
CREATE TABLE DELIVERY(
    DNO NUMBER(6) PRIMARY KEY,
    DNAME VARCHAR2(30) NOT NULL,
    DPHONE VARCHAR2(30) NOT NULL,
...
CREATE TABLE PRODUCT(
    ...
    PRATING NUMBER(4,2) DEFAULT -1 --ÎØ∏Îì±Î°? -1
    );
...
CREATE TABLE CSMG_COMMENT(
    CSNO NUMBER(6) PRIMARY KEY,
    MGID VARCHAR2(30) REFERENCES MANAGER(MGID),
    CNO NUMBER(6) REFERENCES CUSTOMER_SERVICE(CNO),
    CMSUBJECT VARCHAR2(100) NOT NULL,
    CMCONTENT VARCHAR2(4000) NOT NULL,
    CMRDATE DATE DEFAULT SYSDATE
);
CREATE TABLE COUPON(
    CONO NUMBER(6) PRIMARY KEY,
    CNAME VARCHAR2(100) NOT NULL,
    CCONDITION NUMBER(9) NOT NULL, --?ùº?†ïÍ∏àÏï° ?ù¥?ÉÅ Íµ¨Îß§?ãú
    CDISPER NUMBER(3),
    CDISCASH NUMBER(9),
    CORDATE DATE DEFAULT SYSDATE,
    CVALIDITY DATE NOT NULL,
    MID VARCHAR2(30) REFERENCES MEMBER(MID)
);
CREATE TABLE PAYMENT(
    PANO NUMBER(6) PRIMARY KEY,
    ONO NUMBER(6) REFERENCES ORDERS(ONO),
    PRODUCT_ALL_PAY NUMBER(9),
    DISCOUNT NUMBER(9),
    COUPONDISCOUNT NUMBER(9),
    USEPOINT NUMBER(9),
    PURCHASE_AMOUNT NUMBER(9)
);
...
INSERT INTO GRADES (GNO, GNAME, LOWGRADE, HIGHGRADE)
VALUES (3,'VIP',10000001,100000000);
INSERT INTO GRADES (GNO, GNAME, LOWGRADE, HIGHGRADE)
VALUES (4,'VVIP',100000001,999999999);
