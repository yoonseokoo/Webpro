...
public class Ex02_greagorinCalendar {
	public static void main(String[] args) {
		...
		int month = cal.get(Calendar.MONTH) + 1; // 월(Calendar.MONTH)
													// 0,1,2,3...11월
		...
		// %tY(년) %tm(월) %td(일) %ta(요일) %tH(24시) %tp(오전/오후) %tl(12시) %tM(분)
		// %tS(초)
		System.out.printf("%tY년 %tm월 %td일 %ta요일 %tp %tl시 %tM분 %tS초\n", cal, cal,
				cal, cal, cal, cal, cal, cal);
		System.out.printf(
				"%1$tY년 %1$tm월 %1$td일 %1$ta요일 %1$tp %1$tl시 %1$tM분 %1$tS초", cal);
	}
}
